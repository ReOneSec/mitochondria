        ...
        <div className="flex flex-col space-y-3">
          ...
          {facilities.map((f, i) => (
            <Reveal key={f.id} width="100%" delay={i * 50}>
              <div className="group relative grid grid-cols-1 md:grid-cols-12 gap-4 p-6 md:py-8 border border-slate-100 bg-white rounded-2xl transition-all duration-500 ease-out hover:border-emerald-500/30 hover:shadow-xl hover:shadow-emerald-900/5 hover:-translate-y-1 cursor-default items-center">
                ...
                <div className="col-span-2 text-xs uppercase tracking-wider text-slate-500 hidden md:block font-medium">{f.category}</div>
                <div className="col-span-3 text-lg md:text-xl font-bold text-slate-800 group-hover:text-emerald-800 transition-colors duration-300 flex items-center gap-3">
                    {f.title}
                ...
                <div className="col-span-4 text-sm text-slate-500 font-light mt-2 md:mt-0 leading-relaxed group-hover:text-slate-600 transition-colors duration-300">{f.desc}</div>
              </div>
            </Reveal>
          ))}
        </div>
